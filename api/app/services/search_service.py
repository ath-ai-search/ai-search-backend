...
# =========================================================================
# 🌐 HTML MEGA MENU ROUTE (FULLY UPDATED UI)
# =========================================================================
async def get_mega_menu_widget(query_string: str, recent_searches: str = ""):
    ...
    # 1. Always show exactly what they typed at the top
    if clean_query:
        suggestions_html += f"""
        <div class="bclouds-list-item" onclick="document.getElementById('search_query').value='{clean_query}'; document.getElementById('searchBtn').click();">
            ...
            <div class="bclouds-list-text">{clean_query}</div>
        </div>
        """

    # 2. Extract smart text completions from the actual OpenSearch hits
    for hit in hits:
        source = hit.get("_source", {})
        ...
        for c in cats:
            ...
            if c_str and c_str != "none" and c_str not in seen_phrases:
                
                # Only show it if it actually relates to what they typed!
                if clean_query in c_str or not clean_query:
                    seen_phrases.add(c_str)
                    
                    # Highlight the "new" suggested text in bold
                    if clean_query:
                        display_text = c_str.replace(clean_query, f"<span>{clean_query}</span><b>") + "</b>"
                    else:
                        display_text = f"<span>{c_str}</span>"

                    # Mix of image thumbnails and search icons
                    ...
                    suggestions_html += f"""
                    <div class="bclouds-list-item" onclick="document.getElementById('search_query').value='{c_str}'; document.getElementById('searchBtn').click();">
                        {icon_html}
                        <div class="bclouds-list-text">{display_text}</div>
                    </div>
                    """
            if len(seen_phrases) >= 6: break
        if len(seen_phrases) >= 6: break
    ...
    .bclouds-list-icon {{ color: #9ca3af; font-size: 15px; width: 34px; text-align: left; }}
